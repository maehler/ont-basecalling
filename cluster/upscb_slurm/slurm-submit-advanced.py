...
def _get_cluster_configuration(partition):
    ...
    m = re.search("(?P<partition>\S+)\s+(?P<cpus>\d+)\s+(?P<memory>\S+)\s+(((?P<days>\d+)-)?(?P<hours>\d+):(?P<minutes>\d+):(?P<seconds>\d+)|(?P<time>infinite))\s+(?P<size>\S+)\s+(?P<maxcpus>\S+)",
                  res.stdout.decode())
    d = m.groupdict()
    if not 'days' in d or not d['days']:
        d['days'] = 0
    if 'time' in d and d['time'] == 'infinite':
        d['time'] = 14400
    else:
...
slurm_parser.add_argument(
    ...
    default="logs/{rule}.{wildcards}.%j.err" if "logs/{rule}.{wildcards}.%j.err" else None)
slurm_parser.add_argument(
...
slurm_parser.add_argument(
    ...
    default="logs/{rule}.{wildcards}.%j.out" if "logs/{rule}.{wildcards}.%j.out" else None)
slurm_parser.add_argument(
